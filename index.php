    ...
    switch ($act) {
        ...
        case 'themvaogiohang':
            ...
            if(isset($_POST['themgiohang'])){
                extract($_POST);
                $check = check_giohang($id_sanpham_thetich,$user['id']);
                if(is_array($check)){
                    $soluong +=$check['soluong'];
                    update_giohang($soluong,$check['id']);  
                }else{
                    insert_giohang($id_sanpham_thetich,$soluong,$user['id']);
                }
            ...
            break;
        case 'giohang':
            $listgiohang =loadall_giohang($user['id']);
        ...
        case 'thanhtoan':
            //hiển thị
            $listsanpham =loadall_giohang($user['id']);
            $id_giohang="";
            if(isset($_GET['id_giohang'])&&$_GET['id_giohang']){
                $listsanpham =mua1_giohang($user['id'],$_GET['id_giohang']);
                $id_giohang=$_GET['id_giohang'];
            }
        ...
        case 'edit_tk':
            if (isset($_POST['capnhat']) && ($_POST['capnhat'])) {
                ...
                $_SESSION['hoten'] = check_user($hoten, $matkhau);
